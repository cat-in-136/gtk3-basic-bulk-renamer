...
/// Renaming mapping pair with source name and target name.
...
#[derive(Clone, Copy)]
pub enum RenameMapOverwriteMode {
    ChangeFileName,
    Overwrite,
    Error,
}
...
impl RenameMap {
    ...
    pub fn execute(&self, over_write_mode: RenameMapOverwriteMode) -> Result<(), RenameError> {
        self.check_not_found_source_files()?;

        // Step 1 Move the all files to temporary name.
        let mut temp_files = Vec::with_capacity(self.pairs.len());
        for pair in self.pairs.iter() {
            ...
            temp_files.push(target_temp_filename);
        }
        ...
        for (i, pair) in self.pairs.iter().enumerate() {
            if pair.0 != pair.1 {
                let target_file = match over_write_mode {
                    RenameMapOverwriteMode::ChangeFileName => Self::fix_target_file_path(&pair.1),
                    RenameMapOverwriteMode::Overwrite => Ok(pair.1.clone()),
                    RenameMapOverwriteMode::Error => {
                        if pair.1.exists() {
                ...
                fs::rename(&temp_files[i], &target_file)
                    .map_err(|error| RenameError::IoError(pair.clone(), error))?;
            }
        }
...
mod test {
    ...
    pub fn test_fix_target_file_path() {
        ...
        for i in 0..3 {
            ...
            assert_eq!(
                RenameMap::fix_target_file_path(&a_txt).unwrap().file_name(),
    ...
    pub fn test_execute_when_conflicting() {
        for &mode in &[
            RenameMapOverwriteMode::ChangeFileName,
            RenameMapOverwriteMode::Overwrite,
            RenameMapOverwriteMode::Error,
        ] {
            let temp_dir = tempfile::tempdir().unwrap();
            ...
            let rename = RenameMap::new(vec![rename_pair]);
    ...
    pub fn test_execute_with_across_directories() {
        ...
        let rename = RenameMap::new(pairs);
        rename.execute(RenameMapOverwriteMode::Error).unwrap();
